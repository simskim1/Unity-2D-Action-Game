using UnityEngine;

public class EnemyChaseState : IState
{
    private EnemyController enemy;

    public EnemyChaseState(EnemyController enemy)
    {
        this.enemy = enemy;
    }

    public void Enter()
    {
        // 추적(이동) 애니메이션 재생
        enemy.Animator.Play("Enemy_Idle");
    }

    public void Update()
    {
        if (enemy.KnockBackGetter())
            return;
        // 플레이어와의 거리 계산
        float distanceToPlayer = Vector2.Distance(enemy.transform.position, enemy.PlayerTarget.position);

        Collider2D[] hitPlayer = Physics2D.OverlapCircleAll(enemy.attackPoint.position, enemy.attackRange, enemy.playerLayer);
        // 사거리 안쪽이고 & 쿨타임이 다 지났다면? -> 공격!
        if (hitPlayer.Length > 0 && Time.time >= enemy.lastAttackTime + enemy.attackCooldown)
        {
            enemy.StateMachine.TransitionTo(enemy.AttackState);
            return;
        }
        // 사거리 안쪽이긴 한데, 아직 쿨타임이 안 돌았다면? -> 제자리 대기! (부비부비 방지)
        else if (hitPlayer.Length > 0)
        {
            // 멈춰서 플레이어를 째려봄 (속도를 0으로)
            //enemy.Rb.linearVelocity = new Vector2(0f, enemy.Rb.linearVelocity.y);

            // (선택) 여기서 방향(Flip)만 플레이어 쪽을 보게 업데이트 해줄 수도 있어.
            return;
        }
        // 시야 밖으로 벗어남 -> 대기 상태로 전환
        else if (distanceToPlayer > enemy.detectRange)
        {
            enemy.StateMachine.TransitionTo(enemy.IdleState);
            return;
        }

        // 플레이어를 향해 이동 및 방향(Flip) 전환
        MoveTowardsPlayer();
    }

    public void Exit()
    {
        
        // 추적 상태를 벗어날 때(공격하거나 포기할 때) 얼음판처럼 미끄러지지 않게 속도를 0으로 멈춤
        //enemy.Rb.linearVelocity = new Vector2(0f, enemy.Rb.linearVelocity.y);
    }

    private void MoveTowardsPlayer()
    {
        // 방향 구하기: 플레이어가 적보다 오른쪽에 있으면 1, 왼쪽에 있으면 -1
        float directionX = enemy.PlayerTarget.position.x > enemy.transform.position.x ? 1f : -1f;

        // 이동: X축으로는 지정한 속도만큼 이동, Y축 속도(중력/낙하)는 그대로 유지
        enemy.Rb.linearVelocity = new Vector2(directionX * enemy.moveSpeed, enemy.Rb.linearVelocity.y);

        // 방향 뒤집기(Flip): 로컬 스케일의 X값을 방향에 맞춰 1 또는 -1로 변경
        // 주의: 원래 적 오브젝트의 크기가 1이 아니라면, 원본 스케일 값에 맞춰 곱해줘야 해!
        enemy.transform.localScale = new Vector3(directionX * 2, 1f * 3, 1f);
    }
}